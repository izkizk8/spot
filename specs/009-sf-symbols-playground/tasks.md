...
- [ ] T001 Verify `expo-symbols` `~55.0.7` is present in `package.json` and `pnpm-lock.yaml`; if missing, run `npx expo install expo-symbols` from the repo root. Acceptance: `pnpm install` is clean; `pnpm list expo-symbols` reports the SDK 55–compatible version; no other deps changed.
- [ ] T002 [P] Create empty module folders `src/modules/sf-symbols-lab/components/` and `test/unit/modules/sf-symbols-lab/components/`. Acceptance: directories exist; `git status` shows them tracked; nothing else changed.
