...
import type { TintToken } from '@/modules/sf-symbols-lab/types';
...
describe('TintPicker', () => {
  ...
  it('highlights the selected swatch', () => {
    ...
    expect(buttons[0]).toHaveAccessibilityState({ selected: true });
    expect(buttons[1]).toHaveAccessibilityState({ selected: false });
    ...
    expect(updatedButtons[0]).toHaveAccessibilityState({ selected: false });
    expect(updatedButtons[2]).toHaveAccessibilityState({ selected: true });
  });
