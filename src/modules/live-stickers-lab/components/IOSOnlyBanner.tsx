...
export default function IOSOnlyBanner({ style }: IOSOnlyBannerProps) {
  ...
  return (
    <ThemedView testID="live-stickers-ios-only-banner" style={[styles.container, style]}>
      <ThemedText style={styles.icon}>⚠️</ThemedText>
      <ThemedText style={styles.title}>iOS Only Feature</ThemedText>
      <ThemedText style={styles.message}>
        Live Stickers (VNGenerateForegroundInstanceMaskRequest) requires iOS 17+. This feature is
        not available on {currentPlatform}.
      </ThemedText>
    </ThemedView>
  );
}
